...
data.frame(Dose = Dose, y = DR_fun_log_exp(Dose, alpha, beta, NEC)) %>%
...
rho_mat <- matrix(
  c(1, rho_1_2, rho_1_3, rho_1_2, 1, rho_2_3, rho_1_3, rho_2_3, 1),
  nrow = 3
)
...
df.pos.cov <- ID.pos.cov %>%
  expand(nesting(ID = ID.pos.cov, alpha_i, beta_i, NEC_i), Dose = Dose) %>%
...
fig.pos.cov.sd <- df.pos.cov %>%
  summarise(y_i_mu = mean(yhat), y_i_sd = sd(yhat), .by = c(Dose)) %>%
...
df.neg.cov <- ID.neg.cov %>%
  expand(nesting(ID = ID.neg.cov, alpha_i, beta_i, NEC_i), Dose = Dose) %>%
  mutate(log_yhat = DR_fun_log(Dose, alpha_i, beta_i, NEC_i)) %>%
  mutate(yhat = exp(log_yhat)) %>%
  mutate(y = rlnorm(n(), log_yhat, sigma))
...
# Combine figures ----
fig.cor.pos <-
  ggplot(ID.pos.cov, aes(x = NEC_i, y = alpha_i)) +
  stat_ellipse(
    geom = "polygon",
    fill = "#016392",
    alpha = .8,
...
fig.cor.neg <-
  ggplot(ID.neg.cov, aes(x = NEC_i, y = alpha_i)) +
  stat_ellipse(
    geom = "polygon",
    fill = "#c72e29",
    alpha = .8,
...
# ECx - sd relationship ----
df.pos.cov <- df.pos.cov |>
  mutate(cov = "pos", col = "#016392")
df.neg.cov <- df.neg.cov |>
  mutate(cov = "neg", col = "#c72e29")

df.full <- rbind(df.pos.cov, df.neg.cov)

df.full.mu <- summarise(
  df.full,
  y_i_mu = mean(yhat),
  y_i_sd = sd(yhat),
  .by = c(Dose, cov, col)
...
fig.perc_changexsd <- df.full.mu %>%
  ggplot(aes(
    x = abs((y_i_mu - alpha) / alpha * 100),
    y = y_i_sd,
    color = cov
  )) +
  geom_line(linewidth = 2) +
  scale_color_manual(values = c("#c72e29", "#016392")) +
  labs(x = "% change", y = "sd y response") +
  #ylim(0, 1.5) +
  theme_bw(14)
