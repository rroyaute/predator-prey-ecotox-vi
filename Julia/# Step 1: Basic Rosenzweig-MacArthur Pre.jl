...
# Create enhanced phase portrait with equilibria
function plot_phase_portrait_with_equilibria(sol1, sol2, stability_results1, stability_results2, params1, params2)
    
    # Extract trajectories
    resources1 = [u[1] for u in sol1.u]
    consumers1 = [u[2] for u in sol1.u]
    resources2 = [u[1] for u in sol2.u]  
    consumers2 = [u[2] for u in sol2.u]
    ...
    # Plot trajectories
    plot!(p, resources1, consumers1, label="Type I trajectory", lw=2, color=:blue, alpha=0.7)
    plot!(p, resources2, consumers2, label="Type II trajectory", lw=2, color=:red, alpha=0.7)
    
    # Plot equilibria for Type I
    for (i, result) in enumerate(stability_results1)
        eq = result["point"]
        stability = result["stability"]
        
        if stability == "stable node" || stability == "stable spiral"
            marker = :circle
            color = :green
        elseif stability == "unstable node" || stability == "unstable spiral"
            marker = :circle
            color = :red
        elseif stability == "saddle point"
            marker = :diamond
            color = :orange
        else
            marker = :square
        ...
        scatter!(p, [eq[1]], [eq[2]], 
                ...
                color=color, 
                markersize=8, 
                label=i==1 ? "Type I equilibria" : "")
    end
    
    # Plot equilibria for Type II
    for (i, result) in enumerate(stability_results2)
        eq = result["point"]
        stability = result["stability"]
        ...
        scatter!(p, [eq[1]], [eq[2]], 
                ...
                color=color, 
                markersize=8, 
                label=i==1 ? "Type II equilibria" : "")
    end
    
    xlabel!(p, "Resource (R)")
    ylabel!(p, "Consumer (C)")
    title!(p, "Phase Portrait with Equilibrium Analysis")
    
    return p
end

# Create the enhanced plot
enhanced_plot = plot_phase_portrait_with_equilibria(sol, sol2, 
                                                   stability_results_typeI, 
                                                   stability_results_typeII,
                                                   params, params_typeII)
display(enhanced_plot)

println("\n" * "="^60)
println("SUMMARY")
println("="^60)
println("✓ Basic Rosenzweig-MacArthur model implemented")
println("✓ Stability analysis completed")
println("✓ Phase portraits with equilibria plotted")
println("✓ Type I vs Type II functional responses compared")
println("\nStability analysis shows:")
println("- Equilibrium locations and their stability")
println("- Eigenvalue analysis for local behavior")
println("- Phase portrait visualization")

println("\nReady for next step: adding phenotypic variation and numerical integration!")
